...
@property(nonatomic, strong)NSMutableArray *gridListArray;//格子数组
...
- (void)viewDidLoad
{
    ...
    normalImgArr = @[@"home_guardBao",@"home_my_invest",@"home_charge",@"home_withdraw",@"home_add"];
    highlightImgArr = @[@"home_guardBao_tap",@"home_my_invest_tap",@"home_charge_tap",@"home_withdraw_tap",@"home_add_tap"];
    
//    normalImage = [UIImage imageNamed:@"app_item_bg"];//背景图片-普通状态
//    highlightedImage = [UIImage imageNamed:@"app_item_pressed_bg"];//背景图片-按压状态
    deleteIconImage = [UIImage imageNamed:@"app_item_plus"];//删除icon
    
    _showGridArray = [NSMutableArray arrayWithObjects:@"100", @"101",@"102",@"103",@"104",nil];
    
    _moreGridIdArray = [NSMutableArray arrayWithObjects:@"202", @"203", @"204", @"205", @"206", @"207", nil];
    
    
    UIBarButtonItem *rightItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemAdd target:self action:@selector(toTestViewController)];
    
    self.navigationItem.rightBarButtonItem = rightItem;
    
}

- (void)toTestViewController
{
...
{
    int column;
    int count = (int)_showGridArray.count;
    if (count % 3 == 0) {
        column = count / 3;
    }else
    {
    ...
    NSLog(@"count == %d column == %d",count,column);
    
    _gridListView = [[UIView alloc] init];
    [_gridListView setFrame:CGRectMake(0, 60, ScreenWidth, GridHeight * column)];
    ...
    for (NSInteger index = 0; index < [_showGridArray count]; index++)
    {
        ...
        UIImage *normalImg = [UIImage imageNamed:normalImgArr[index]];
        UIImage *highlightImg = [UIImage imageNamed:highlightImgArr[index]];
        
        CustomGrid *gridItem = [[CustomGrid alloc] initWithFrame:CGRectZero title:nil normalImage:normalImg highlightedImage:highlightImg gridId:[gridTitle integerValue] atIndex:index isAddDelete:isAddDelete deleteIcon:deleteIconImage];
...
//            UIImage *normalImg = [UIImage imageNamed:normalImgArr[i]];
...
- (void)pressGestureStateBegan:(UILongPressGestureRecognizer *)longPressGesture withGridItem:(CustomGrid *) grid
{
    ...
    if (!isSelected) {
        ...
        UIImage *highlightImg = [UIImage imageNamed:highlightImgArr[gridId - 100]];
        //给选中的格子添加放大的特效
        [UIView animateWithDuration:0.5 animations:^{
            grid.transform = CGAffineTransformMakeScale(1.2, 1.2);
            grid.alpha = 0.8;

            [grid setBackgroundImage:highlightImg forState:UIControlStateNormal];
        }];
    }
}
